...
| A3 **P0** | **Regex → circuit compilation** | Today the regex is evaluated onchain over circuit-revealed text. Production should compile each market's pattern with zk-regex into the circuit so only the boolean match (plus minimal captures) is public. Needs a per-market verifying-key registry and a compile step at market creation. |
...
| # | Item | Notes |
|---|---|---|
| E1 **P0** | **Gas reality pass** | The no-gas-constraints assumption must go for mainnet: regex evaluation moves into the circuit (A3), evidence to events/IPFS, markets become clones (EIP-1167) instead of 42KB deployments. An L2/appchain keeps the rest viable as-is. |
