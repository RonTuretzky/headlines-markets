    ...
    constructor(
        ...
        for (uint256 i = 0; i < sources_.length; i++) {
            ...
            string memory effective =
                bytes(sources_[i].contentRegex).length > 0 ? sources_[i].contentRegex : _contentRegex;
            sourceFromPatternHash[i] = keccak256(bytes(sources_[i].fromRegex));
            sourceContentPatternHash[i] = keccak256(abi.encodePacked(uint8(_contentField), effective));
        }
        require(
