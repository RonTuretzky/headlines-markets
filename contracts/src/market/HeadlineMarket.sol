...
contract HeadlineMarket {
    ...
    constructor(
        ...
        string memory _contentRegex,
        ContentField _contentField,
        Source[] memory sources_,
        uint8 _threshold,
        uint64 _windowStart,
        uint64 _deadline,
        uint64 _resolutionBuffer
    ) {
        require(sources_.length > 0 && sources_.length <= 32, "Market: 1-32 sources");
        require(_threshold > 0 && _threshold <= sources_.length, "Market: bad threshold");
        require(_deadline > block.timestamp, "Market: deadline in past");
        require(_deadline > _windowStart, "Market: deadline before window start");

        // Validate every regex now so malformed patterns can never brick settlement.
        if (bytes(_contentRegex).length > 0) _contentRegex.validate();
        for (uint256 i = 0; i < sources_.length; i++) {
            require(bytes(sources_[i].dkimDomain).length > 0, "Market: empty domain");
            ...
            for (uint256 j = 0; j < i; j++) {
                require(
                    keccak256(bytes(sources_[i].dkimDomain)) != keccak256(bytes(sources_[j].dkimDomain)),
                    "Market: duplicate source domain"
                );
            }
            if (bytes(sources_[i].fromRegex).length > 0) sources_[i].fromRegex.validate();
            if (bytes(sources_[i].contentRegex).length > 0) sources_[i].contentRegex.validate();
            _sources.push(sources_[i]);
        }
        require(
            bytes(_contentRegex).length > 0 || allSourcesHaveOverrides(sources_),
            "Market: no content condition"
        );

        question = _question;
        description = _description;
        contentRegex = _contentRegex;
        contentField = _contentField;
    ...
    function submitProof(uint256 sourceIndex, EmailProof calldata proof) external {
        ...
        if (bytes(src.fromRegex).length > 0) {
            require(src.fromRegex.test(proof.fromAddress), "Market: from address mismatch");
        }
        ...
        returns (bool ok, string memory reason)
    {
        ...
        if (bytes(src.fromRegex).length > 0 && !src.fromRegex.test(proof.fromAddress)) {
    ...
    function contentMatches(Source storage src, EmailProof calldata proof) internal view returns (bool) {
        ...
        if (contentField == ContentField.Subject) return pattern.test(proof.subject);
        if (contentField == ContentField.Body) return pattern.test(proof.bodyExcerpt);
        return pattern.test(proof.subject) || pattern.test(proof.bodyExcerpt);
    }
